...
export async function fetchJob(jobId) {
  ...
  return {
    ...
    failReason: status === "FAILED"
      ? (d.failReason || d.error || data.message || d.musics?.[0]?.failReason || "Unknown Suno error")
      : undefined
  };
}
...
// ── Reference (Sample-to-Song) ────────────────────────────────────────
// Full flow: buffer → temp public URL → TTAPI upload → music_id → sample job.
// tempStore must be a Map managed by the caller (server.js) so the GET handler
// can serve the file while TTAPI fetches it.

export async function submitSampleFromBuffer(buffer, mime, tempStore, publicBase, opts = {}) {
  if (!ttapiEnabled()) throw new Error("TTAPI_KEY not configured");

  // 1. Store buffer temporarily (5 min TTL)
  const { randomUUID } = await import("node:crypto");
  const id = randomUUID();
  tempStore.set(id, { buffer, mime, expires: Date.now() + 5 * 60 * 1000 });

  // 2. Upload to TTAPI via our public URL
  const audioUrl = `${publicBase}/api/tmp/${id}`;
  console.log("[ttapi-upload] sending url:", audioUrl);
  const uploadData = await post("/suno/v1/upload", { audio_url: audioUrl, is_async: false });
  const musicId = uploadData.data?.music_id || uploadData.data?.musicId;
  console.log("[ttapi-upload] raw response:", JSON.stringify(uploadData).slice(0, 300));
  if (!musicId) throw new Error(`Upload returned no music_id — raw: ${JSON.stringify(uploadData).slice(0, 200)}`);

  // 3. Generate sample-to-song
  const chopStart = Number(opts.startSec ?? 0);
  const chopEnd = Number(opts.endSec ?? Math.min(60, chopStart + 30));
  // custom=true only when user actually provides lyrics (not just style tags)
  const hasLyrics = !!(opts.lyrics && String(opts.lyrics).trim());
  const body = {
    music_id: musicId,
    chop_sample_start_s: chopStart,
    chop_sample_end_s: chopEnd,
    custom: hasLyrics,
  ...
  if (opts.tags) body.tags = String(opts.tags).slice(0, 1000);
  if (hasLyrics && !opts.instrumental)
    body.prompt = String(opts.lyrics).slice(0, 5000);
  // Always set gpt_description_prompt for inspiration / style context
  body.gpt_description_prompt = String(opts.descriptionPrompt || opts.tags || "").slice(0, 3000);
  if (opts.audioWeight !== undefined) body.audio_weight = opts.audioWeight;
  if (opts.styleWeight !== undefined) body.style_weight = opts.styleWeight;
  if (opts.vocalGender) body.vocal_gender = opts.vocalGender;
  if (opts.hookUrl) body.hookUrl = opts.hookUrl;

  console.log("[ttapi-sample] body:", JSON.stringify(body).slice(0, 400));
  const sampleData = await post("/suno/v1/sample", body);
  console.log("[ttapi-sample] response:", JSON.stringify(sampleData).slice(0, 300));
  // Clean up temp entry after submission (TTAPI already fetched it)
  setTimeout(() => tempStore.delete(id), 30000);
  return { jobId: sampleData.data?.job_id || sampleData.data?.jobId };
}
