...
// ─── #9 Style Genome ─────────────────────────────────────────────────────
// Blend 2-3 artists proportionally → hybrid Suno prompt.

const GENOME_SYSTEM =
  "You are a music geneticist who cross-breeds artist styles for Suno AI v5.5. " +
  "Given 2-3 artists with percentage weights, you create a hybrid style prompt that " +
  "combines their production DNA proportionally. " +
  "The dominant artist (highest %) sets the genre skeleton. " +
  "Minority artists contribute specific flavours: an instrument texture, a rhythmic feel, " +
  "a vocal characteristic, or a harmonic colour — weighted by their percentage. " +
  "The result must be a coherent, singable style — not a laundry list. " +
  "Never name real artists. Reply JSON only.";

export async function styleGenome(artists, catalog) {
  // artists = [{name, prompt, weight}]  weights should sum to ~100
  if (!aiEnabled()) return { ok: false, error: "AI provider not configured" };
  if (artists.length < 2) return { ok: false, error: "Need at least 2 artists" };

  const totalWeight = artists.reduce((s, a) => s + a.weight, 0);
  const artistDesc = artists.map((a) =>
    `${a.weight}% ${a.name}: [${a.prompt || "no prompt"}]`
  ).join("\n");

  const user =
    `Cross-breed these artists by weight (total ${totalWeight}%):\n${artistDesc}\n\n` +
    `Build a single coherent Suno style prompt that captures:\n` +
    `- Genre/era skeleton from the dominant artist (${artists[0].name})\n` +
    `- Specific flavours from minority artists proportional to their weight\n` +
    `- Resolve conflicts: if artists clash on tempo, take weighted average BPM\n` +
    `- The prompt should feel like ONE coherent style, not a mashup list\n\n` +
    `Respond JSON:\n` +
    `{"prompt": string, "bpm": number|null, "key": string, "genre": string,\n` +
    `"mood": string[], "instruments": string[], "vocals": string,\n` +
    `"dna_breakdown": [{"artist": string, "contribution": string, "weight": number}],\n` +
    `"concept": string}`;

  const json = await llmJSON(GENOME_SYSTEM, user, 1000);
  if (!json?.prompt) return { ok: false, error: "AI returned no genome" };
  return {
    ok: true,
    prompt: json.prompt,
    bpm: json.bpm || null,
    ...
    instruments: Array.isArray(json.instruments) ? json.instruments : [],
    vocals: json.vocals || "",
    dnaBreakdown: Array.isArray(json.dna_breakdown) ? json.dna_breakdown : [],
...
// ─── #10 Playlist Builder ─────────────────────────────────────────────────
// One style/concept → N tracks with coherent album arc.

const PLAYLIST_SYSTEM =
  "You are a music director creating a cohesive album. " +
  "Given a style and optional theme, generate N tracks that form a natural album arc: " +
  "opener → build → peak → bridge → closer. " +
  "Each track shares the core sonic DNA but has a distinct mood and energy. " +
  "Prompts must be 8-15 tokens: genre, instruments, mood, tempo, key. No real artist names. " +
  "Reply JSON only.";

export async function buildPlaylist(style, theme, count = 8) {
  if (!aiEnabled()) return { ok: false, error: "AI provider not configured" };
  const user =
    `Create an album of ${count} tracks.\n` +
    `Core style: ${style}\n` +
    (theme ? `Album theme: ${theme}\n` : "") +
    `\nEach track: unique title, tight Suno style prompt (8-15 tokens), ` +
    `mood tag, BPM, key, and role (opener/build/peak/bridge/closer).\n` +
    `Progress energy: opener is calm/atmospheric, build tracks gain intensity, ` +
    `peak is the highest energy, bridge is an emotional pivot, closer is resolution.\n\n` +
    `Respond JSON: {"albumTitle": string, "concept": string, "tracks": [` +
    `{"number": 1, "title": string, "prompt": string, "mood": string, "bpm": number, "key": string, "role": string}` +
    `]}`;
  const json = await llmJSON(PLAYLIST_SYSTEM, user, 2500);
  if (!Array.isArray(json?.tracks) || json.tracks.length === 0) {
    return { ok: false, error: "AI returned no tracks" };
  }
  return {
    ok: true,
    albumTitle: json.albumTitle || style,
    concept: json.concept || "",
    tracks: json.tracks.map((tr, i) => ({
      number: tr.number ?? i + 1,
